...
class TextClozeBatch:
    ...
    context_panel_vit_input: Optional[Dict]
...
class TextClozeDataset(Dataset):
    def __init__(
        self,
        ...
        self.fold_dict = None
        if fold in ('dev', 'test'):
            self.fold_dict = read_fold_text_cloze(
...
def read_fold_text_cloze(csv_file, vdict, max_len=30):
    """
